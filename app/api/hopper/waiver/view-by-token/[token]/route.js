...
City Tour Guide Inc. — Tampa, FL — 833-813-8687 — citytourguide.app`;
...
function buildGuestHtml(w, ride) {
  ...
  const rideInfo = ride ? `
    ...
    <div class="field-row">
      ...
      <span class="field-value">${escapeHtml([ride.neighborhood, ride.venue_name].filter(Boolean).join(' — ') || '—')}</span>
    </div>
...
<head>
  ...
  <title>Signed Waiver — ${escapeHtml(w.guest_name || 'Guest')} — CTG</title>
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .back-btn{display:block;width:100%;margin-top:10px;padding:13px;background:#fff;color:#374151;border:1.5px solid #E5E7EB;border-radius:12px;font-size:0.88rem;font-weight:700;cursor:pointer;font-family:inherit;text-align:center;text-decoration:none}
...
<body>
  <div class="page">
    ...
    <div class="header no-print">
      ...
      <div>
        <div class="header-sub">Tampa · City Hopper</div>
        <div class="header-title">Signed Liability Waiver</div>
      </div>
    </div>

    <div class="badge">✅ Waiver Signed</div>
    ...
    <button class="print-btn no-print" onclick="window.print()">📄 Download / Print Signed Waiver</button>
    <a href="javascript:history.back()" class="back-btn no-print">← Back</a>

    <div class="footer no-print">
      City Tour Guide Inc. · Tampa, FL · 833-813-8687 · citytourguide.app<br>
      Your signed waiver document.
    </div>
...
export async function GET(req, { params }) {
  ...
  if (!token) {
    return new Response('<h2>Missing waiver token.</h2>', {
  ...
  const { data: w, error } = await supabaseAdmin
    .from('hopper_waivers')
    .select('id, token, request_id, guest_name, guest_phone, signed_at, signature_data, dob, email, address, city, state, zip, waiver_type, minors')
    .eq('token', token)
    .single();
  ...
  if (error || !w) {
    if (acceptJson) return NextResponse.json({ error: 'Waiver not found' }, { status: 404 });
    return new Response('<h2>Waiver not found. Please use the original link from your confirmation.</h2>', {
      status: 404, headers: { 'Content-Type': 'text/html' },
    });
  }

  if (!w.signed_at) {
    if (acceptJson) return NextResponse.json({ error: 'This waiver has not been signed yet.' }, { status: 404 });
    return new Response('<h2>This waiver has not been signed yet.</h2>', {
      status: 404, headers: { 'Content-Type': 'text/html' },
    });
  }

  // ── JSON mode: used by client-side generateWaiverPDF on Hopper request page ──
  if (acceptJson) {
    let ride = null;
    if (w.request_id) {
      const { data: r } = await supabaseAdmin
        .from('hopper_requests')
        .select('neighborhood, venue_name, guest_count')
        .eq('id', w.request_id).single();
      ride = r || null;
    }
  ...
  // ── HTML fallback: direct browser navigation / print ──
  let ride = null;
  if (w.request_id) {
    const { data: r } = await supabaseAdmin
      .from('hopper_requests')
      .select('neighborhood, venue_name, dropoff_notes, guest_count')
      .eq('id', w.request_id).single();
    ride = r || null;
  }
  const html = buildGuestHtml(w, ride);
