...
const VENUES = {
  ...
  'Downtown Tampa': [
    ...
    'Dick Greco Plaza Transit Center',
  ],
  'Channelside': [
    'Sparkman Wharf',
    'Florida Aquarium',
    'Port of Tampa Parking Garage',
  ],
  'Water Street': [
    'Publix - Water Street',
        ...
        {NEIGHBORHOODS.map(n => (
          <button type="button" key={n} onClick={() => { onSelect(n); onLandmark(''); }}
            style={{padding:'10px 8px',borderRadius:12,border:'1.5px solid',cursor:'pointer',fontWeight:600,fontSize:'0.78rem',fontFamily:'Inter,sans-serif',transition:'all 0.15s',
              borderColor:selected===n?'#00FF88':'#E5E7EB',background:selected===n?'#0B1D3A':'#fff',color:selected===n?'#00FF88':'#374151'}}>
            {n}
          </button>
        ))}
      </div>
...
function RequestFormInner() {
  ...
  // On-demand riders skip the timing step — start directly at Pickup (step 2)
  ...
  const guests = adults + children; // total, kept for legacy compat
  const [price, setPrice]   = useState(6);

  const [submitting, setSubmitting] = useState(false);
  const [error, setError]           = useState('');
  // Wave H: duty status check
  const [dutyStatus, setDutyStatus] = useState(null); // null=loading, {is_on_duty:bool} when known


  // Reset submitting state if user navigates back from waiver page
  useEffect(() => {
    const handleBack = () => setSubmitting(false);
    window.addEventListener('pageshow', handleBack);
    return () => window.removeEventListener('pageshow', handleBack);
  }, []);
  useEffect(() => {
    fetch('/api/hopper/duty-status', { cache: 'no-store' })
      ...
      .then(d => {
        setDutyStatus(d);
        // Auto-switch to scheduled if Off Duty and currently on ondemand
        if (!d.is_on_duty && params.get('type') !== 'scheduled') {
          setType('scheduled');
          setStep(1);
        }
      })
      .catch(() => {}); // fail silently — don't block form
  }, []);

  useEffect(() => {
    try {
      const s = JSON.parse(localStorage.getItem('ctg_hopper_info') || '{}');
      // Also read home-page keys (ctg_hop_*) as fallback so riders who used the home form are pre-filled
      ...
      if (n) setName(n);
      if (p) setPhone(p);
      if (g > 1) setAdults(g); // legacy: treat as adults
  ...
  async function submit() {
    ...
    // Wave H: re-check duty before on-demand submission
    if (type === 'ondemand') {
      try {
      ...
      } catch { /* duty check failed — let server guard catch it */ }
    }
    ...
    try {
      ...
      if (d.id && d.waiver_skipped) {
        ...
        // Handle 409 Off Duty rejection from server
  ...
  return (
    <div style={{minHeight:'100vh',background:'#F9FAFB',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{padding:'20px 16px',maxWidth:500,margin:'0 auto'}}>
        ...
        {/* Wave H: Off Duty warning banner */}
        ...
        {step===3 && (
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            ...
            {/* Pre-fill notice — shown when name/phone are restored from localStorage */}
            {hasSavedInfo
        ...
        {step===5 && (
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            ...
            {type === 'ondemand' && dutyStatus !== null && !dutyStatus.is_on_duty ? (
              <div style={{background:'#FEF3C7',border:'1.5px solid #F59E0B',borderRadius:14,
                padding:'16px',textAlign:'center',display:'flex',flexDirection:'column',gap:10}}>
                <div style={{fontWeight:800,fontSize:'0.9rem',color:'#92400E'}}>
                  No Drivers on Duty at This Time
                </div>
                <div style={{fontSize:'0.8rem',color:'#92400E',lineHeight:1.5}}>
                  Schedule a future Hop instead. Pick a date and time that works for you.
                </div>
                <button type="button" onClick={() => { setType('scheduled'); setStep(1); }}
                  style={{padding:'13px',borderRadius:12,border:'none',background:'#F59E0B',
                    color:'#fff',fontWeight:800,fontSize:'0.9rem',cursor:'pointer',
                    fontFamily:'Inter,sans-serif',boxShadow:'0 4px 12px rgba(245,158,11,0.35)'}}>
                  📅 Schedule in Advance
                </button>
              </div>
            ) : (
              <button type="button" onClick={submit} disabled={submitting || guests > 5}
                ...
                {submitting?'Redirecting to waiver...':'🛺 Reserve My Hop'}
              </button>
            )}
          </div>
        )}
        ...
        {step<5 && (
          <div style={{display:'flex',gap:10,marginTop:20}}>
            ...
            <button type="button" onClick={goNext} disabled={!canNext}
              style={{flex:2,padding:'13px',borderRadius:14,fontWeight:800,fontSize:'0.95rem',cursor:canNext?'pointer':'not-allowed',fontFamily:'Inter,sans-serif',transition:'all 0.2s',
                border:canNext?'1.5px solid #00FF88':'none',background:canNext?'#0B1D3A':'#E5E7EB',color:canNext?'#00FF88':'#9CA3AF',boxShadow:canNext?'0 0 12px rgba(0,255,136,0.2)':'none'}}>
      ...
      <div style={{textAlign:'center',marginTop:16,fontSize:'0.65rem',color:'#9CA3AF',lineHeight:1.6,maxWidth:320,margin:'16px auto 0',paddingBottom:8}}>City Tour Guide connects guests with featured local destinations, attractions, dining, entertainment, and curated city experiences.</div>
...
export default function RequestPage() {
  return (
    <Suspense fallback={<div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh',fontFamily:'Inter,sans-serif',color:'#6B7280',fontSize:'1.2rem'}}>⏳</div>}>
      <RequestFormInner />
    </Suspense>
  );
}